...
def reject_legacy_rls_runner():
    # This shared-host runner cannot create the isolated temporary roles needed
    # by the RLS suite. Never elevate its service account to work around that.
    raise RuntimeError(
        "Legacy QA runner disabled: use an exclusive PostgreSQL test cluster with synthetic data; see docs/rls.md"
    )
...
def validate_existing(commit):
    ...
    plan(commit)
    reject_legacy_rls_runner()
    if os.geteuid() != 0:
        raise RuntimeError(
            "Administrative account required for an exclusive test database"
        )
    import pwd
    ...
    if (
        credentials.stat().st_uid != account.pw_uid
    ...
    if (
        ...
        connection.username,
        connection.path,
    ) != ("postgresql", "127.0.0.1", 5432, ROLE, "/nabio_elege_qa"):
        raise RuntimeError("QA database target mismatch")
    privileges = postgres(
        "SELECT rolsuper OR rolcreatedb OR rolcreaterole OR rolreplication OR rolbypassrls FROM pg_roles WHERE rolname='nabio_elege_qa';"
    )
    if privileges != "f":
    ...
    if (
        ...
        or postgres(f"SELECT datname FROM pg_database WHERE datname='{database}';")
    ):
        raise RuntimeError(
            "Validation revision already exists; preserve it and inspect"
        )
    ...
    with os.fdopen(
        os.open(backup, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600), "wb"
    ) as output:
        run(
            [
                "runuser",
    ...
    run(
        ["git", "clone", "--no-checkout", ORIGIN, str(release)],
        timeout=120,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.PIPE,
    )
    run(
        ["git", "-C", str(release), "checkout", "--detach", commit],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.PIPE,
    )
    actual = run(
        ["git", "-C", str(release), "rev-parse", "HEAD"], capture_output=True, text=True
    ).stdout.strip()
    if actual != commit:
    ...
    run(
        [
            *command,
            ...
            "--no-cache-dir",
            "-r",
            str(release / "requirements-production.lock"),
        ],
        timeout=600,
    )
    ...
    result = subprocess.run(
        [
            *command,
            ...
            "--keepdb",
            "--noinput",
            "--settings=nabio_elege.postgres_test_settings",
        ],
        cwd=release,
        env=env,
        timeout=600,
    ...
    print(
        json.dumps(
            {"test_exit_code": result.returncode, "public_services_created": False}
        ),
        flush=True,
    )
...
def provision(commit):
    ...
    if os.geteuid() != 0:
        raise RuntimeError(
            "Administrative account required for isolated resource creation"
        )
    import pwd
    ...
    conflicts = postgres(
        "SELECT datname FROM pg_database WHERE datname IN ('nabio_elege_qa', 'test_nabio_elege_qa'); SELECT rolname FROM pg_roles WHERE rolname='nabio_elege_qa';"
    )
    if conflicts:
    ...
    postgres(
        "SET password_encryption='scram-sha-256'; CREATE ROLE nabio_elege_qa LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION PASSWORD '"
        + password
        + "';"
    )
    for database in DATABASES:
        ...
        postgres(
            f"REVOKE ALL ON DATABASE {database} FROM PUBLIC; GRANT CONNECT ON DATABASE {database} TO nabio_elege_qa;"
        )
    environment = {
        "SECRET_KEY": secrets.token_urlsafe(64),
        "TEST_DATABASE_URL": f"postgresql://{ROLE}:{password}@127.0.0.1:5432/nabio_elege_qa",
        "PYTHONUNBUFFERED": "1",
        "PYTHONDONTWRITEBYTECODE": "1",
    }
    credentials_path = ROOT / "shared/qa-environment.json"
    with os.fdopen(
        os.open(credentials_path, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600), "w"
    ) as output:
    ...
    print(
        json.dumps(
            {
                "backup_before_test_migrations": backups,
                "note": "Empty dedicated databases; archive structure verified, restore drill still pending",
            }
        ),
        flush=True,
    )
    ...
    run(
        ["runuser", "-u", ACCOUNT, "--", "python3.12", "-m", "venv", str(venv)],
        timeout=120,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--existing",
        action="store_true",
        help="Validate a new revision in the previously provisioned exclusive QA environment",
    )
    args = parser.parse_args()
    try:
        sys.exit(
            validate_existing(args.commit) if args.existing else provision(args.commit)
        )
            ...
            json.dumps(
                {
                    ...
                    "instruction": "Inspect the exclusive QA resources; do not delete or blindly rerun",
                }
            ),
            flush=True,
        )
