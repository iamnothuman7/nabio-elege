...
from infra.validate_isolated import (
    plan,
    provision,
    validate_existing,
    validation_database,
...
class IsolatedProvisioningTests(SimpleTestCase):
    def test_legacy_runner_cannot_touch_resources_even_as_admin(self):
        for operation in (provision, validate_existing):
            with (
                self.subTest(operation=operation.__name__),
                patch(
                    "infra.validate_isolated.os.geteuid", return_value=0, create=True
                ),
                patch("infra.validate_isolated.run") as run,
                patch("infra.validate_isolated.postgres") as postgres,
            ):
                with self.assertRaisesRegex(RuntimeError, "Legacy QA runner disabled"):
                    operation("a" * 40)
    ...
    def test_non_admin_is_rejected_before_any_provisioning(self):
        with (
            patch("infra.validate_isolated.os.geteuid", return_value=1000, create=True),
            patch("infra.validate_isolated.run") as run,
        ):
    ...
    def test_revalidation_database_has_an_exclusive_bounded_name(self):
        self.assertEqual(
            validation_database("a" * 40), "test_nabio_elege_qa_aaaaaaaaaaaa"
        )
